...
export const PurchasePay = ({ changeScreen, order_id }: Props) => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      } else changeScreen('purchasedone');
    } catch (err) {
      errorModal(err);
    }
    setOnLoad(false);
      ...
      <div className='flex h-[500px] flex-col gap-4 overflow-y-scroll scrollbar-hide'>
        <div className='my-12 flex h-fit w-full flex-col items-center justify-center gap-4 text-center !font-proxima'>
          ...
          <div className='max-w-[350px] text-gray-500'>
            ...
            {role?.includes(Role.USER) && <a href={`/account/purchases/${order_id}`}>Go to Order</a>}
            {role?.includes(Role.TEMP) && <a href={`/guest/purchases/${order_id}`}>Go to Order</a>}
          </div>
        </div>
